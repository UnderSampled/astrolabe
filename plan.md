...
**Progress:** Steps **1–8 complete**. **Semantic dual-layer** (post–Step 8): animation families/transforms plus scene, geometry, AI, characters, sectors — nested authoring docs + **only** `astrolabe.sna-block-content.v2` segments/expand (no v1 `elements[]` fallback). **Step 9 partial:** GPT/PTX/SDA/SND live in `sidecars/level.json` (wire-lossless; not opaque `files/` SoT); `texture:/` / `sound:/` resolve; PNG corpus still needs full CNT walk; PNG→GF / WAV→BNM rebuild and codec-driven RTP/RTT/RTS still open. Step **10** deferred (LZO encoder parity). Framework: [`notes/semantic-dual-layer-framework.md`](notes/semantic-dual-layer-framework.md); residuals: [`notes/residual-opaque.txt`](notes/residual-opaque.txt).
...
**In progress (partial).** GPT/PTX/SDA/SND are imported into `sidecars/level.json` (`WireBase64` + texture/sound URI inventories); opaque `files/*.{gpt,ptx,sda,snd}` are deleted after aggregate; export regenerates loose files from WireBase64. Remaining Step 9 work: full mirrored `Gamedata/` tree on import by default, referenced PNG/WAV decode at scale, PNG→GF / WAV→BNM on export, and RTP/RTT/RTS from codec pointer metadata (not heuristic scans).
